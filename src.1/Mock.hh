...
final class Mock<TC> implements MockInterface {
	...
	public function __construct(
        private classname<TC> $interface_name,
        private ?Vector<string> $unmockedMethods
    ): void {
	...
	public function build(): TC {
		...
		foreach ($rfl->getMethods() as $method) {
            ...
            if (
                $this->unmockedMethods !== null &&
                $this->unmockedMethods->linearSearch($method_name) > -1
            ) {
                $f = $method->getFileName();
                $start_line = (int) $method->getStartLine() - 1;
                ...
                $body = '';
                for( $i = $start_line; $i < $end_line - 2; $i++) {
                    $body.="{$source[$i]}\n";
                }
